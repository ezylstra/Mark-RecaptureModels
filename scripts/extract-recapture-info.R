...
# Check out dates to see whether Aug/Sep captures are transients --------------#
...
first_cap <- bird_months %>%
  group_by(first_cap) %>%
  summarize(n_indiv = n(),
            n_recaptured = sum(n_captures > 1),
            n_multyears = sum(n_years > 1),
            prop_recaptured = n_recaptured / n_indiv,
            prop_multyears = n_multyears / n_indiv)
first_cap          
  # 6697/10131 first captured in MJJ; 534 first captured in Sep
  # Proportion of birds captured in multiple years
    # Birds first captured in MMJ = 19%
    # Birds first captured in Aug 1-15 = 16%
    # Birds first captured in Aug 16-31 = 13%
    # Birds first captured in Sep = 5%
